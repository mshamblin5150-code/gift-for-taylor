...
2. Subscribe a device-local iPhone Calendar, an iCloud-located Calendar, and a Google Calendar *From URL* entry. For Google sharing, use two throwaway Google accounts with the **same** test URL and a third with a **different** URL. Keep the feed content unchanged during the baseline. A 7-day baseline captures long Google gaps and day-of-week effects.
