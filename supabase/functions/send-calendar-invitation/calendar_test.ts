...
import { invitationCalendar, invitationMessage, type Invitation } from "./calendar.ts";
...
Deno.test("Cancellation retains the event identity and advances its sequence", () => {
  ...
  if (!result.includes("METHOD:CANCEL\r\n") ||
...
Deno.test("Email carries an iMIP request rather than a calendar import", async () => {
  ...
  if (!raw.includes("Content-Type: text/calendar; charset=utf-8; method=REQUEST")) {
  ...
  if (!cancelled.message.toString().includes(
